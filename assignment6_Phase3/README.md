...
![Garden Sequencer UI](./garden_sequencer_ui.png)

---

### New UI Button System

I added a complete UI layer for selecting flower types.
The old version had no instrument selection, and all flowers used the same shape and synth.

- created a UIButton class
- added shape-based drawing (circle, square, rectangle, triangle)
- an auto-layout function so buttons line up correctly
- added a hit-testing method (isHit) for detecting UI clicks
- separated UI clicks from planting logic so the two no longer conflict

This makes instrument selection stable and prevents accidental flower creation when clicking the UI.

---

### Renamed and Reorganized the Four Instrument Types

I reorganized the four flower types so they each represent a distinct instrument.
Their names are now:
Bloom (circular petals)
Sharp (square petals)
Glyph (rectangular petals)
Thorn (triangular petals)
Each type now has its own hue value, its own drawing logic, and its own Tone.js synth configuration.

---

### Completed the Synth Mapping

In Version 2 only one synth existed.
This week I added full mappings so each type uses a different synthesis method:

- Bloom uses a triangle-based PolySynth
- Sharp uses a square-wave PolySynth
- Glyph uses an AM-based PolySynth
- Thorn uses an FM-based PolySynth

I also fixed an issue where the old LEAD type created an undefined synth, which caused volume errors.
All four types now initialize correctly and respond to volume changes.

---

### Integrated Instrument Type Into Planting Logic

Version 2 always planted the same kind of flower.
Now planting uses currentFlowerType, which is updated by the UI panel.
This required cleaning up the mousePressed logic so UI detection happens before flower creation.

---

### Color Mapping Updated to Match New Types

Hue values are now tied to the new names:
Bloom uses green
Reed uses blue
Facet uses purple
Thorn uses orange
And pitch maps to brightness, petal color is brighter when pitch is higher.

---

### Summary

This version focuses on UI interface and structural clarity.

The sequence of changes includes:

- a new button-based instrument selection panel
- four instrument types and synth mapping for each type
- clearer separation between UI interaction and flower planting
- updated hue system
